...
> Les menu du programme utilise une nouvelle version non publié de la fonction de dialog
#### Commandes:
...
- SIX pour passer au mode suivant
- FOUR pour passer au mode précent 
...
<details>
<summary>Modifier le fichier de configuration</summary>

<br>
Modifier le fichier de configuration de l'application peut être interrésant pour changer les limites par défauts. <br><br>
Vous pouvez modifier les variables suivantes:
<ul>
<li>`state` permet de mettre par défaut le fond du dessin, la couleur par défaut, la taille du curseur, sa position et le mode.</li>
<li> `taille_min` et `taille_max` dans la def `scale_menu` pour pouvoir modifier les limites de la taille du curseur. </li>
...
## Applications
### [Game Of Life](https://my.numworks.com/python/itzrayanis/gameoflife) 
Jeu de la vie de conway.
> (Utilise la même base que le programme de dessin)
#### Commandes:
- Flèches directionnels pour bouger le curseur
- OK pour écrire/effacer
- SHIFT pour alterner entre écrire/effacer
- TOOLBOX pour accéder au menu de configuration (Utiliser les flèches directionnels et OK pour valider)
- NINE pour passer à la prochaine génération
